                        ...
                        case K of
                            {key_tx_error, KeyTxErr} ->
                                error_logger:error_msg("Invalid key on load ~p: ~p",
                                                       [K0, KeyTxErr]);
                            {K1, KeyMeta = #keymeta{}} ->
                                case is_key_expired(KeyMeta#keymeta.tstamp_expire, Now) of
                                    false ->
...
expired_keys_on_startup_01_test() ->
    ...
    RemainingKeysValues =
        lists:foldl(
            fun({K, V}, A) ->
                {K1, Meta} = KT(K),
                case bitcask:get(B1, K1) of
                    {ok, V} ->
                        T = Meta#keymeta.tstamp_expire,
                        [{<<T:32/integer, K1/binary>>, V}|A];
                    _ ->
                        A
                end
            end, [], AllKeyValues),
    ExpectedKeyValues = NotExpiredDeletes ++ NormalPuts,
    ?assertEqual(lists:sort(ExpectedKeyValues), lists:sort(RemainingKeysValues)).
